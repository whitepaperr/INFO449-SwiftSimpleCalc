...
func calculate(_ args: [String]) -> Int {
    if args.count == 3 {
            guard let firstOperand = Int(args[0]), let secondOperand = Int(args[2]) else { return 0 }
            switch args[1] {
            ...
            let sum = args.dropLast().reduce(0) { $0 + (Int($1) ?? 0) }
            let count = args.count - 1
            return count > 0 ? sum / count : 0
        } else if args.last == "fact" {
            if let n = Int(args.first ?? ""), n >= 0 {
                return n == 0 ? 1 : (1...n).reduce(1, *)
            } else {
                return 0 // In case the input is not a valid integer or negative
            }
        }
        return 0
}
...
func calculate(_ args: [String]) -> Double {
    if args.count == 3 {
            guard let firstOperand = Double(args[0]), let secondOperand = Double(args[2]) else { return 0.0 }
            switch args[1] {
            case "+": return firstOperand + secondOperand
            case "-": return firstOperand - secondOperand
            case "*": return firstOperand * secondOperand
            case "/": return firstOperand / secondOperand
            case "%": return firstOperand.truncatingRemainder(dividingBy: secondOperand)
            default: return 0.0
            }
        } else if args.last == "count" {
            ...
            let sum = args.dropLast().reduce(0.0) { $0 + (Double($1) ?? 0.0) }
